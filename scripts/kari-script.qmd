...
high_density_species <- ggplot(
  ...
  geom_bar_pattern(
    stat = "identity",
    position = position_dodge2(width = 0.9, padding = 0, preserve = "single"),
    color = "black",
    width = 0.85,
    pattern_fill = "black",
    pattern_density = 0.35,
...
xaxis_label <- textGrob("Density~(m^-2)", gp = gpar(fontsize = 12))
...
grid.arrange(
  yaxis_label, combined_species_density, 
  ncol = 2, 
  widths = c(0.05, 0.95),
  bottom = xaxis_label)
...
*Plotting the density of key kelp grazers at sites throughout the Salish Sea*
...
#density/60m^2
inverts_comparison_sites_kelpgrazers <- inverts_comparison_sites_kelpgrazers %>% 
  mutate(density_60m2 = amount,
         density_m2 = amount / 60)
...
inverts_comparison_sites_kelpgrazers <- inverts_comparison_sites_kelpgrazers %>% 
  ...
  summarise(
    mean_density_60m2 = mean(density_60m2, na.rm = TRUE),
    sd_density_60m2 = sd(density_60m2, na.rm = TRUE),
    mean_density_m2 = mean(density_m2, na.rm = TRUE),
    sd_density_m2 = sd(density_m2, na.rm = TRUE),
    n_transects = n(),
    .groups = "drop")
...
ggplot(inverts_comparison_sites_kelpgrazers, 
  ...
  geom_errorbar(aes(
    ymin = pmax(mean_density_60m2 - sd_density_60m2, 0),
    ymax = mean_density_60m2 + sd_density_60m2),
    width = 0.2,
    position = position_dodge(width = 0.8)) +
  facet_wrap(~ site_name, scales = "free_x") +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 5)) + 
  scale_fill_manual(values = c("2024" = "gray",
                               "2025" = "blue")) +
